...
class DownloadPool():
    ...
    def StopLoop(self, loop):
        asyncio.run_coroutine_threadsafe(self.CheckDone(self.DownLoop),
                                         self.DownLoop)

    def Start(self):
        self.DownLoop = asyncio.new_event_loop()
        self.DownThread = threading.Thread(target=self.StartLoop,
                                           args=(self.DownLoop, ))
    ...
    def Download(self, url, filename):
        asyncio.run_coroutine_threadsafe(self.AsyncDownload(url, filename),
                                         self.DownLoop)
