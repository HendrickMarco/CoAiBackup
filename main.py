...
def Retry(func,
    ...
    while (times):
        try:
            ...
            fg = 1
    if (fg):
        raise RetryCheckFailed(func.__qualname__, args, cfunc.__qualname__,
                               resp)
    # else:
...
def TiebaRequest(url, data, first=False):
    ...
    if (int(ret["error_code"]) != 0):
        raise RequestError({
            "code": int(ret["error_code"]),
            "msg": str(ret["error_msg"])
        })
    return req.json()
...
def ProcessComment(author, t, content):
    return '%s | %s:<blockquote>%s</blockquote>' % (FormatTime(t), author,
                                                    content)
...
def GetComment(floor, pid, fid):
    ...
    while (1):
        ...
        for comment in data:
            Write(
                ProcessComment(comment["author"]["name_show"], comment["time"],
...
def GetTitle(pid):
    data = TiebaRequest(const.PageUrl, {
        "kz": str(pid),
        "_client_version": "9.9.8.32"
    }, True)
    return {
        "post": data["post_list"][0]["title"],
...
def GetPost(pid, lz, comment):
    ...
    while (1):
        ...
        for floor in data["post_list"]:
            ...
            fid = int(floor["id"])
            Write(
                ProcessFloor(fnum, floor["author"]["name"], floor["time"],
